...
async function nikitaWeather() {
  const res = await fetch("https://api.weatherapi.com/v1/current.json?key=457c7925b2764ff8bed82246241610&q=Munich&aqi=no");

  const data = await res.json();

  const icon = data.current.condition.icon;

  const shortRegion = data.location.region.slice(0, 2).toUpperCase()
//   console.log(shortCountry)

  cityText.textContent = `${data.location.name}, ${shortRegion}`;
  ...
  ${data.location.localtime.slice(8, 11)}
  `;
  // Для вызова года!
  /* ${todayDate.getFullYear().toString().slice(-2)} */
 
  const res2 = await fetch(
    "https://api.weatherapi.com/v1/forecast.json?key=457c7925b2764ff8bed82246241610&q=Munich&days=7&aqi=no&alerts=no"
  );
